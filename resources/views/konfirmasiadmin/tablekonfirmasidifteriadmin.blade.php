...
<head>
    ...
    <title>Status Suspek Difteri</title>
...
<body class="m-0 font-sans antialiased font-normal text-base leading-default bg-gray-50 text-slate-500">
    ...
    <main class="ease-soft-in-out xl:ml-68.5 relative h-full max-h-screen rounded-xl transition-all duration-200">
        ...
        <div class="p-4">
            <form method="GET" action="{{ route('searchdifteriadmin') }}">
                <div class="col-md-1">
        ...
        <div class="p-4">
            <a href="{{route('exportsuspekdifteri')}}" class="px-4 py-2.5 bg-utama text-white font-bold rounded-md">Export Excel</a>
        </div>
        ...
        <div class="flex overflow-auto p-4">
            <table class="text-sm text-black w-full">
                <thead class="text-xs text-black bg-gray-300">
                    ...
                    <th scope="col" class="px-2 py-4">STATUS SEMPEL</th>
                    <th scope="col" class="px-2 py-4">JENIS KELAMIN</th>
                    <th scope="col" class="px-2 py-4">TANGGAL LAHIR</th>
                    <th scope="col" class="px-2 py-4">NAMA SEKOLAH/TEMPAT KERJA</th>
                ...
                @foreach ($join as $item)
                    <tr class="bg-white border-b text-xs">
                        ...
                        <td class="px-6 py-4 bg-green-200">
                            {{ $item->getSuspekdifteridanKonfirmasidifteri ? $item->getSuspekdifteridanKonfirmasidifteri->noepid : '' }}
                        </td>
                        <td class="px-6 py-4 bg-green-200">
                            {{ $item->getSuspekdifteridanKonfirmasidifteri ? $item->getSuspekdifteridanKonfirmasidifteri->tanggalterimadkk : '' }}
                        </td>
                        <td class="px-6 py-4 bg-green-200">
                            {{ $item->getSuspekdifteridanKonfirmasidifteri ? $item->getSuspekdifteridanKonfirmasidifteri->status : '' }}
                        </td>
                        <td class="px-6 py-4 bg-green-200">
                            {{ $item->getSuspekdifteridanKonfirmasidifteri ? $item->getSuspekdifteridanKonfirmasidifteri->hasil : '' }}
                        </td>
                        <td class="px-6 py-4 bg-green-200">
                            {{ $item->getSuspekdifteridanKonfirmasidifteri ? $item->getSuspekdifteridanKonfirmasidifteri->tanggalhasilkeluar : '' }}
                        </td>
                        <td class="px-6 py-4 bg-green-200">
                            {{ $item->getSuspekdifteridanKonfirmasidifteri ? $item->getSuspekdifteridanKonfirmasidifteri->dasarsurat : '' }}
                        </td>
                        <td class="px-6 py-4 bg-green-200">
                            {{ $item->getSuspekdifteridanKonfirmasidifteri ? $item->getSuspekdifteridanKonfirmasidifteri->rtl : '' }}
                        </td>
                        <td class="px-6 py-4">
                            <a href="/konfirmasiadmin/{{ $item->id }}/editdifteri"
