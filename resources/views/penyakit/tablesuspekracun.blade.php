...
<head>
    ...
    <title>Table Suspek Suspek Keracunan Makanan</title>
    ...
    <main class="ease-soft-in-out xl:ml-68.5 relative h-full max-h-screen rounded-xl transition-all duration-200">
        ...
        <div class="flex overflow-auto p-4">
            <table class="text-sm text-black w-full">
                ...
                <tbody
                    @foreach ($susracun as $item)
                            <tr class="bg-white border-b">
                                <th class="px-6 py-4">{{ $item['id'] }}</th>
                                <td class="px-6 py-4">{{ $item['puskesmas'] }}</td>
                                <td class="px-6 py-4">{{ $item['namaindekkasus'] }}</td>
                                <td class="px-6 py-4">{{ $item['lokasikejadian'] }}</td>
                                <td class="px-6 py-4">{{ $item['kelurahan'] }}</td>
                                <td class="px-6 py-4">{{ $item['kecamatan'] }}</td>
                                <td class="px-6 py-4">{{ $item['tanggalinformasi'] }}</td>
                                ...
                                <td class="px-6 py-4">{{ $item['jenissempel'] }}</td>
                                <td class="px-6 py-4">
                                    <a href="{{route('createkonfirmasiracun')}}" class="px-6 py-4 bg-utama rounded-xl font-bold text-white hover:bg-gray-300 hover:text-white">konfirmasi</a>
                                </td>
                            </tr> @endforeach
                    </tbody>
            </table>
